...
import { FiUpload, FiFile, FiDownload, FiSettings, FiTrash2, FiCheck, FiGrid } from 'react-icons/fi';
...
export default function ExcelToPdfPage() {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files.length > 0) {
      ...
      const isExcelFile = /\.(xls|xlsx|csv)$/i.test(selectedFile.name);
      if (!isExcelFile) {
        setError('Please select an Excel spreadsheet (.xls, .xlsx, .csv).');
        return;
      }
  ...
  const processSpreadsheet = async () => {
    if (!file) {
      setError('Please select an Excel spreadsheet.');
      return;
    }
    ...
    try {
      ...
      formData.append('includeGridlines', includeGridlines.toString());
      formData.append('fitToPage', fitToPage.toString());
      
      // Send request to API
      const response = await fetch('/api/convert/document/excel-to-pdf', {
        method: 'POST',
        body: formData,
      });
      ...
      if (downloadLinkRef.current) {
        ...
        downloadLinkRef.current.download = file.name.replace(/\.(xls|xlsx|csv)$/i, '.pdf');
        setIsComplete(true);
      }
    } catch (err: any) {
      console.error('Error converting spreadsheet:', err);
      setError(err.message || 'An error occurred while converting the spreadsheet. Please try again.');
    } finally {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-3xl font-bold mb-2">Convert Excel to PDF</h1>
      <p className="text-gray-600 dark:text-gray-300 mb-6">
        Transform your Excel spreadsheets into professional PDF files with perfect formatting
      </p>
      
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        {/* Left column - File upload */}
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6">
          <h2 className="text-xl font-semibold mb-4">1. Select Excel Spreadsheet</h2>
          ...
          {!file ? (
            ...
            >
              ...
              <div className="flex flex-col items-center justify-center">
                ...
                <p className="text-xs text-gray-500 dark:text-gray-600">
                  Accepted formats: .xls, .xlsx, .csv • Maximum file size: 30MB
                </p>
              </div>
            </div>
          ) : (
            <div className="flex items-center space-x-4">
              <div className="flex items-center flex-1 min-w-0">
                <div className="flex items-center justify-center w-10 h-10 mr-3 bg-green-100 dark:bg-green-900/30 text-green-600 dark:text-green-400 rounded-full">
                  <FiFile size={20} />
                </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6">
          ...
          <div className="space-y-6">
            ...
            <div className="flex items-center">
              ...
              />
              <label htmlFor="includeGridlines" className="ml-2 block text-sm text-gray-700 dark:text-gray-300">
                Include gridlines in PDF
              </label>
            </div>
            ...
            <div className="flex items-center">
              <input
                ...
                className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
              />
              <label htmlFor="fitToPage" className="ml-2 block text-sm text-gray-700 dark:text-gray-300">
            ...
            <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg mt-4">
              ...
              <ul className="text-xs text-blue-600 dark:text-blue-400 space-y-1 list-disc pl-5">
                <li>Preserves all cell data and formatting</li>
                <li>Maintains charts, tables, and graphs</li>
                <li>Supports multi-sheet documents</li>
                <li>Scales content to fit page if needed</li>
                <li>Optionally includes gridlines and headers</li>
              </ul>
            </div>
          </div>
        </div>
      ...
      {isComplete && (
        <div className="mt-8 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-xl p-6 animate-fadeIn">
          ...
          <p className="text-green-700 dark:text-green-300 mb-4">
            Your Excel spreadsheet has been converted to PDF with {quality} quality.
          </p>
