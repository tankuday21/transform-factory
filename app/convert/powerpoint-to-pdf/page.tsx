...
import { FiUpload, FiFile, FiDownload, FiSettings, FiTrash2, FiCheck, FiLayers, FiFileText } from 'react-icons/fi';
...
export default function PowerPointToPdfPage() {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files.length > 0) {
      ...
      const isPowerPointFile = /\.(ppt|pptx)$/i.test(selectedFile.name);
      if (!isPowerPointFile) {
        setError('Please select a PowerPoint presentation (.ppt or .pptx).');
        return;
      }
  ...
  const validatePageRange = (range: string): boolean => {
    ...
    // Examples: "1,3,5-7,9" or "1-5" or "1,3,5"
  ...
  const processPresentation = async () => {
    ...
    if (!validatePageRange(pageRange)) {
      setError('Invalid page range format. Use formats like "1-5", "1,3,5" or "all".');
      return;
    }
    ...
    try {
      ...
      // Send request to API
      const response = await fetch('/api/convert/document/powerpoint-to-pdf', {
        method: 'POST',
        body: formData,
      });
      ...
      if (downloadLinkRef.current) {
        ...
        downloadLinkRef.current.download = file.name.replace(/\.(ppt|pptx)$/i, '.pdf');
        setIsComplete(true);
      }
    } catch (err: any) {
      console.error('Error converting presentation:', err);
      setError(err.message || 'An error occurred while converting the presentation. Please try again.');
    } finally {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-3xl font-bold mb-2">Convert PowerPoint to PDF</h1>
      <p className="text-gray-600 dark:text-gray-300 mb-6">
        Transform your PowerPoint presentations into high-quality PDF files with perfect formatting
      </p>
      
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        {/* Left column - File upload */}
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6">
          <h2 className="text-xl font-semibold mb-4">1. Select PowerPoint Presentation</h2>
          ...
          {!file ? (
            ...
            >
              ...
              <div className="flex flex-col items-center justify-center">
                ...
                <p className="text-gray-600 dark:text-gray-400 mb-2">
                  Drag and drop your PowerPoint presentation here, or{' '}
                  <button
                    type="button"
                ...
                <p className="text-xs text-gray-500 dark:text-gray-600">
                  Accepted formats: .ppt, .pptx • Maximum file size: 30MB
                </p>
              </div>
            </div>
          ) : (
            <div className="flex items-center space-x-4">
              <div className="flex items-center flex-1 min-w-0">
                <div className="flex items-center justify-center w-10 h-10 mr-3 bg-orange-100 dark:bg-orange-900/30 text-orange-600 dark:text-orange-400 rounded-full">
                  <FiFile size={20} />
                </div>
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
                Higher quality preserves all formatting but results in larger file size
              </p>
            </div>
            ...
            <div className="flex items-center">
              ...
              />
              <label htmlFor="includeNotes" className="ml-2 block text-sm text-gray-700 dark:text-gray-300">
            ...
            <div>
              ...
              />
              <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
                Specify slide numbers or ranges to include, or use 'all' for all slides
              </p>
            </div>
            ...
            <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg mt-4">
              ...
              <ul className="text-xs text-blue-600 dark:text-blue-400 space-y-1 list-disc pl-5">
                <li>Preserves all slide layouts and formatting</li>
                <li>Maintains images, charts, and graphics</li>
                <li>Keeps animations as static elements</li>
                <li>Optionally includes speaker notes</li>
                <li>Converts slides to high-quality PDF pages</li>
              </ul>
            </div>
          </div>
        </div>
      ...
      {isComplete && (
        <div className="mt-8 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-xl p-6 animate-fadeIn">
          ...
          <p className="text-green-700 dark:text-green-300 mb-4">
            Your PowerPoint presentation has been converted to PDF with {quality} quality.
          </p>
