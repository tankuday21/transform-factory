...
export default function WordToPdfPage() {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files.length > 0) {
      ...
      const isWordFile = /\.(doc|docx)$/i.test(selectedFile.name);
      if (!isWordFile) {
  ...
  const processDocument = async () => {
    ...
    try {
      ...
      // Send request to API
      const response = await fetch('/api/convert/document/word-to-pdf', {
        method: 'POST',
        body: formData,
      });
      ...
      if (downloadLinkRef.current) {
        ...
        downloadLinkRef.current.download = file.name.replace(/\.(doc|docx)$/i, '.pdf');
        setIsComplete(true);
      }
    } catch (err: any) {
      console.error('Error converting document:', err);
      setError(err.message || 'An error occurred while converting the document. Please try again.');
    } finally {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <p className="text-gray-600 dark:text-gray-300 mb-6">
        Transform your Word documents into professional PDF files with perfect formatting
      </p>
      
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        {/* Left column - File upload */}
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6">
          <h2 className="text-xl font-semibold mb-4">1. Select Word Document</h2>
            ...
            >
              ...
              <div className="flex flex-col items-center justify-center">
                ...
                <p className="text-xs text-gray-500 dark:text-gray-600">
                  Accepted formats: .doc, .docx • Maximum file size: 30MB
                </p>
              </div>
            </div>
          ) : (
          ...
          <div className="space-y-6">
            ...
            <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg mt-4">
              ...
              <ul className="text-xs text-blue-600 dark:text-blue-400 space-y-1 list-disc pl-5">
                <li>Maintains all formatting, fonts, and styles</li>
                <li>Preserves images, tables, and charts</li>
                <li>Keeps headers, footers, and page numbers</li>
                <li>Creates PDF with searchable text</li>
                <li>Supports hyperlinks and cross-references</li>
              </ul>
            </div>
          </div>
        </div>
      ...
      {isComplete && (
        <div className="mt-8 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-xl p-6 animate-fadeIn">
          ...
          <p className="text-green-700 dark:text-green-300 mb-4">
            Your Word document has been converted to PDF with {quality} quality.
          </p>
