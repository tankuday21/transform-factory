    ...
    try {
      ...
      try {
        ...
        if (pageRange === 'all') {
          ...
          if (pageSet.size === 0) {
            console.error('To JPG: Invalid page range provided');
            return NextResponse.json(
        ...
        if (Object.keys(zip.files).length === 0) {
          console.error('To JPG: No pages could be converted');
          return NextResponse.json(
            { error: 'Failed to convert any pages to JPG' },
            { status: 500 }
          );
        }
